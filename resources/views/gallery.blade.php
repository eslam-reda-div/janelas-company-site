...
<head>
    ...
    <title>{{ app(App\Settings\SiteSetting::class)->title }} - Gallery</title>
...
{{--                                        @endforeach--}}
        ...
        <div class="page-title-content">
            <div class="container">
                <div class="blog-title color-white">
                    <h2>{{ __('gallery') }}</h2>
                    <span><a href="{{ route('home') }}" style="{{ App::isLocale('ar') ? 'direction: ltr;margin-left: 10px;' : '' }}" class="hv-color-st1 color-white"> {{ __('home') }} </a>{{ __('gallery') }}</span>
                </div>
            </div>
        </div>
    </div><!-- Page-title -->
    ...
    <section class="portfolio text-center">
        <div class="container portfolio-style">
            <div class="tabs-case-study tabs-garallry">
                <div class="flat-tabs tabs-case flat-tabs-type2">
                    <ul class="menu-tab menu-tab-style">
                        <li class="active">
                            <a href="#">{{ __('all') }}</a>
                        </li>
                        @foreach(app(App\Settings\GallerySetting::class)->images as $imageSec)
                            <li>
                                <a href="#">
                                    {{ App::isLocale('ar') ? $imageSec['category_ar'] : $imageSec['category_en'] }}
                                </a>
                            </li>
                        @endforeach
                    </ul>
                    <div class="content-tab content-tab-gallery">
                        @php
                            $allImages = array();

                            foreach (app(App\Settings\GallerySetting::class)->images as $imageSec){
                                foreach ($imageSec['images'] as $image){
                                    $allImages[] = $image;
                                }
                            }
                        @endphp
                        <div class="content-inner">
                            <div class="col-tab align-items-center">
                                @foreach($allImages as $value => $image)
                                    <div class="image-profolio style2 {{ (($value + 1) % 3) == 0 ? 'no-margin-right' : $value }}">
                                        <div class="image">
                                            <img loading="lazy"
                        ...
                        @foreach(app(App\Settings\GallerySetting::class)->images as $imageSec)
                            <div class="content-inner">
                                <div class="col-tab align-items-center">
                                    @foreach($imageSec['images'] as $value => $image)
                                        <div class="image-profolio style2 {{ (($value + 1) % 3) == 0 ? 'no-margin-right' : $value  }}">
                                            <div class="image">
                                                <img loading="lazy"
                                                    src="{{ URL::asset('storage/' . $image) }}"
        ...
        <div class="themesflat-spacer clearfix" data-desktop="90" data-mobile="60" data-smobile="50" ></div>
    </section>
...
<!-- Mirrored from themesflat.co/html/janelas/page-gallery.html by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 30 Apr 2024 15:04:29 GMT -->
</html>
