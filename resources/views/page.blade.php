...
<div class="animsition boxed blog">
        ...
        <div class="page-title-content">
            <div class="container">
                <div class="blog-title color-white">
                    <h2>{{ App::isLocale('ar') ? $page->title_ar : $page->title_en }}</h2>
                    <span><a href="{{ route('home') }}" style="{{ App::isLocale('ar') ? 'direction: ltr;margin-left: 10px;' : '' }}" class="hv-color-st1 color-white"> {{ __('home') }} </a>{{ App::isLocale('ar') ? $page->title_ar : $page->title_en }}</span>
                </div>
            </div>
        </div>
    </div><!-- Page-title -->
    ...
    <section class="container custom-style">
        {!! $page->content !!}
    </section>
...
<script>
    function addSubNonAllClass(className) {
        // Get all elements with the specified class name
        const elements = document.querySelectorAll("." + className);

        // Iterate through each element
        elements.forEach(element => {
            // Add the "sub-non-all" class to all children elements recursively
            const addSubNonAll = (el) => {
                // Add class to current element
                el.classList.add("sub-non-all");
                // Iterate through child elements
                el.childNodes.forEach(child => {
                    // Check if the child is an element node
                    if (child.nodeType === 1) {
                        // Recursively add class to child element
                        addSubNonAll(child);
                    }
                });
            };

            // Start adding class to children
            addSubNonAll(element);
        });
    }

    // addSubNonAllClass('custom-style')
</script>
</body>

<!-- Mirrored from themesflat.co/html/janelas/page-door.html by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 30 Apr 2024 15:04:16 GMT -->
</html>
