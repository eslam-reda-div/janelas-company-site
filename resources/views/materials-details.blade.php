...
<head>
    ...
    <title>{{ app(App\Settings\SiteSetting::class)->title }} - Materials</title>
        ...
        <div class="page-title-content">
            <div class="container">
                <div class="blog-title color-white">
                    <h2>{{ App::isLocale('ar') ? $material->title_ar : $material->title_en }}</h2>
                    <span><a href="{{ route('home') }}" style="{{ App::isLocale('ar') ? 'direction: ltr;margin-left: 10px;' : '' }}" class="hv-color-st1 color-white"> {{ __('home') }} </a>{{ App::isLocale('ar') ? $material->title_ar : $material->title_en }}</span>
                </div>
            </div>
        </div>
    </div><!-- Page-title -->
                    ...
                    <div id="sidebar sidebar-service">
                        <div id="inner-sidebar" class="inner-content-wrap">
                            ...
                            <div class="widget widget-style2 widget-category widget-bg">
                                <h2 class="widget-title-style2 widget-title-style2 "><span>{{ __('popular-materials') }}</span></h2>
                                <ul class="category-wrap wow fadeIn" data-wow-duration="1s" data-wow-delay="0.4s">
                                    @foreach(\App\Models\Material::where('is_published', 1)->whereNot('id', $material->id)->limit(5)->orderByDesc('id')->get() as $popMaterial)
                                        <li>
                                            <div class="block-inside">
                                                <a href="{{ route('materials-details', $popMaterial->id) }}">{{ App::isLocale('ar') ? $popMaterial->title_ar : $popMaterial->title_en }}</a>
                                            </div>
                                        </li>
                                    @endforeach
                                </ul>
                            ...
                            <div class="image-box-style3 bg-color">
                                <div class="image">
                                    <img loading="lazy" src="{{ URL::asset('storage/' . app(App\Settings\SiteSetting::class)->catalog_image) }}" alt="images">
                                </div>
                                <i class="icon-Icon-metro-file-pdf"></i>
                                <div class="content-image-box">
                                    <div class="tftitle-style2" style="{{ App::isLocale('ar') ? 'direction: rtl;' : '' }}">
                                        {{ __('company-profile') }}
                                    </div>
                                    <p class="color-white" style="{{ App::isLocale('ar') ? 'direction: rtl;' : '' }}">
                                        {!! App::isLocale('ar') ? app(App\Settings\SiteSetting::class)->catalog_paragraph_ar : app(App\Settings\SiteSetting::class)->catalog_paragraph_en !!}
                                    </p>
                                    <div class="btn-dowload">
                                        <a download href="{{ URL::asset('storage/' . app(App\Settings\SiteSetting::class)->catalog_pdf) }}">{{ __('download') }}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="site-content-style2">
                        <div class="image-hover ">
                            <div class="image border-5">
                                <img loading="lazy" style="width: 100% !important;" src="{{ URL::asset('storage/' . $material->image) }}" alt="images">
                            </div>
                        </div>
                        <div class="title-section wow fadeInDown"  style="{{ App::isLocale('ar') ? 'direction: rtl;' : '' }}">
                            <div class="flat-title">
                                {{ App::isLocale('ar') ? $material->title_ar : $material->title_en }}
                            </div>
                        </div>

                        <div class="title-section wow fadeInDown"  style="{{ App::isLocale('ar') ? 'direction: rtl;' : '' }}">
                            <div class="flat-title" style="font-size: 25px !important;">
                                SAR {{ $material->unit_price }}
                            </div>
                        </div>
                    </div>
                    <div class="box-details">
                        {!! App::isLocale('ar') ? $material->description_ar : $material->description_en !!}
                    </div>

                    @php
                        $href = 'https://wa.me/+' . app(App\Settings\SiteSetting::class)->whats_app_number . '?text=' . urlencode('أريد طلب هذه الخامه: ' . $material->title_ar);
                    @endphp
                    <a style="margin-top: 35px;width: 100%" href="{{ $href }}" class="themesflat-button btn-style-one" target="_blank">
                        <span class="btn-title">
                            {{ __('order-the-material') }}
                        </span>
                    </a>
                </div>
            </div>
...
<!-- Mirrored from themesflat.co/html/janelas/service-details.html by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 30 Apr 2024 15:04:23 GMT -->
</html>
